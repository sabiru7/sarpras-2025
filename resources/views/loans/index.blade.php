...
@section('content')
    <div class="p-6">
        ...
        <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-6 gap-4">
            <div>
                <h1 class="text-xl font-semibold text-gray-800">Manajemen Peminjaman</h1>
                <p class="text-sm text-gray-500 mt-1">Daftar permohonan peminjaman barang</p>
            </div>
        ...
        <div class="mb-6 flex flex-wrap gap-2">
            ...
            <a href="{{ route('loans.index', ['status' => 'approved']) }}"
                class="px-3 py-1 rounded-full text-sm font-medium {{ request()->get('status') === 'approved' ? 'bg-green-100 text-green-800' : 'bg-gray-100 text-gray-800 hover:bg-gray-200' }}">
                <i class="fas fa-check-circle mr-1"></i> Disetujui
            </a>
            <a href="{{ route('loans.index', ['status' => 'rejected']) }}"
        ...
        <div class="bg-white rounded-xl border border-gray-100 shadow-sm overflow-hidden">
                    ...
                    <table class="min-w-full divide-y divide-gray-100">
                        <thead class="bg-gray-50">
                            <tr>
                                ...
                                <th scope="col"
                                    class="px-5 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">
                                    <i class="far fa-calendar-alt mr-1"></i> Pinjam
                                </th>
                                <th scope="col"
                                    class="px-5 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">
                                    <i class="far fa-calendar-check mr-1"></i> Kembali
                                </th>
                                <th scope="col"
                                    class="px-5 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">
                        ...
                        <tbody class="bg-white divide-y divide-gray-100">
                            @foreach ($loans as $loan)
                                <tr class="hover:bg-gray-50 transition-colors">
                                    ...
                                    <td class="px-5 py-4 whitespace-nowrap">
                                        <div class="text-sm text-gray-800">{{ $loan->item->name }}</div>
                                    ...
                                    <td class="px-5 py-4 whitespace-nowrap">
                                        ...
                                        @else
                                            <span
                                                class="px-2.5 py-1 inline-flex text-xs font-medium rounded-full bg-red-100 text-red-800">
                                                <i class="fas fa-times-circle mr-1"></i> Ditolak
                                            </span>
                                        @endif
                                    </td>
            ...
            @else
                <div class="p-8 text-center">
                    <i class="fas fa-hand-holding text-3xl text-gray-300 mb-3"></i>
                    <h3 class="mt-2 text-sm font-medium text-gray-700">Tidak ada data peminjaman</h3>
                    <p class="mt-1 text-sm text-gray-500">Belum ada permohonan peminjaman barang</p>
                </div>
            @endif
        </div>
